...
def main():
	...
	try:
		...
		print(f"### Step {program_state.current_step}: {current_time} fs ".ljust(66, "-"))
...
def end_conditions_met(program_state):
	...
	if program_state.max_steps is not None and program_state.current_step >= program_state.max_steps:
		return True
	return False
...
def print_structure(program_state):
	...
	for atom, position in zip(program_state.atoms, program_state.structures[-1].as_angstrom()):
		print(f"    {atom.symbol.ljust(2)} {position[0]:15.6f} {position[1]:15.6f} {position[2]:15.6f}")
...
def print_accelerations(program_state):
	...
	for atom, acceleration in zip(program_state.atoms, program_state.accelerations[-1].as_meter_per_sec_sqrd()):
		print(f"    {atom.symbol.ljust(2)} {acceleration[0]:15.6e} {acceleration[1]:15.6e} {acceleration[2]:15.6e}")
...
def print_forces(program_state):
	...
	for atom, force in zip(program_state.atoms, program_state.forces[-1].as_newton()):
		print(f"    {atom.symbol.ljust(2)} {force[0]:15.6e} {force[1]:15.6e} {force[2]:15.6e}")
