...
def _sample(zero_point_energies, program_state):
    ...
    for f in range(len(program_state.frequencies)):
        if program_state.temperature == 0:
            ...
            zpe_ratio = math.exp(
                (-2 * zero_point_energies.as_kcal_per_mole(f))
    ...
    for mode in program_state.fixed_vibrational_quanta:
        ...
        vibrational_quantum_numbers[mode - 1] = program_state.fixed_vibrational_quanta[
            mode
        ]
...
def _calculate_displacement(
    ...
    for f in range(len(program_state.frequencies)):
        ...
        max_shift = math.sqrt(
            2
            * mode_energies.as_millidyne_angstrom(f)
        ...
        if program_state.frequencies.as_recip_cm(f) > 10:
            if (
                ...
                is enums.GeometryDisplacement.EDGE_WEIGHTED
            ):
                random_weight = program_state.random.edge_weighted()
            elif (
                program_state.geometry_displacement_type
                is enums.GeometryDisplacement.GAUSSIAN_DISTRIBUTION
            ):
                random_weight = program_state.random.gaussian()
            elif (
                program_state.geometry_displacement_type
                is enums.GeometryDisplacement.UNIFORM
            ):
...
def _geometry_displacement(shifts, program_state):
    ...
    for f in range(len(program_state.frequencies)):
        for j in range(program_state.number_atoms):
            ...
            program_state.structures[0].alter_position(
                j, new_x, new_y, new_z, enums.DistanceUnits.ANGSTROM
            )
    return
...
def _add_rotational_energy(atomic_velocities, program_state):
    ...
    if eRotX >= 1:
        kinetic_rotational_X = (
            math.log(1 - program_state.random.uniform())
            * -0.5
            * sc.GAS_CONSTANT_KCAL
            * program_state.temperature
        )
    if eRotY >= 1:
    ...
    rotational_kinetic_energy.append(
        kinetic_rotational_X + kinetic_rotational_Y + kinetic_rotational_Z,
...
def generate(program_state):
    ...
    if program_state.energy_boost is enums.EnergyBoost.ON:
        ...
        print(
            f"  energy is between {program_state.energy_boost_min} and "
    ...
    if program_state.geometry_displacement_type is not enums.GeometryDisplacement.NONE:
        ...
        for atom, position in zip(
            program_state.atoms, program_state.structures[0].as_angstrom()
        ):
            print(
                f"    {atom.symbol.ljust(2)} {position[0]:10.6f} "
    ...
    for i, (mode_energy, quantum_n, frequency, direction) in enumerate(
        ...
        print(
            f"  {i:>4}  {frequency:10.3f}  {quantum_n:>11}  "
    ...
    if program_state.add_rotational_energy is enums.RotationalEnergy.YES:
        rotational_kinetic_energy = _add_rotational_energy(
            atomic_velocities, program_state
        )
        print(
            f"  {rotational_kinetic_energy.as_kcal_per_mole(0):.6f} kcal/"
            "mol rotational energy added."
        )
        total_kinetic_energy = _calculate_kinetic_energy(
            atomic_velocities, program_state
        )
    else:
    ...
    excitation_energy.append(
        total_mode_energy.as_kcal_per_mole(0) - total_zpe.as_kcal_per_mole(0),
        enums.EnergyUnits.KCAL_PER_MOLE,
    )
