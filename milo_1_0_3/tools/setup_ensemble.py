...
def main():
    ...
    for line in args.forward:
        if "$job" in line.casefold():
            ...
            in_job_section
            and "program" in line.casefold()
            and "gaussian09" in line.casefold()
        ):
            gaussian_string = "g09"
    ...
    if args.backward is not None:
        ...
        if not args.no_script and forward_memory == "memory_placeholder":
            print(
                "Warning: Could not find memory specification in backward "
                "template file. Replace 'memory_placeholder' in submission "
                "script before submitting jobs."
            )
        if not args.no_script and forward_procs == "processors_placeholder":
    ...
    if not args.no_script and args.time == "time_placeholder":
        print(
            "Warning: Time not specified. Replace 'time_placeholder' in "
            "submission script before submitting jobs."
        )
    ...
    for _ in range(args.number_trajectories):
        ...
        if args.backward is not None:
            forward_name = input_file_basename + "_" + str(random_seed) + "_fwd.in"
            make_input_file(forward_name, forward_template, random_seed)
            backward_name = input_file_basename + "_" + str(random_seed) + "_bwd.in"
            ...
            name = input_file_basename + "_" + str(random_seed) + ".in"
...
def make_submission_script(file_name, time_string, memory, procs, gaussian_string):
    ...
    with open(file_name, "w") as file:
        ...
        file.write("pid=$!\n")
        file.write(
            '# Associate the function "cleanup" with the TERM signal, '
            "which is usually\n"
        )
