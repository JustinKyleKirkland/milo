...
def main():
	...
	parser.add_argument(
		...
		"--time",
		required=True,
		help="Amount of time the submission script should request, formatted as HH:MM:SS.",
	)
	parser.add_argument(
		"-m",
		"--memory",
	...
	parser.add_argument(
		"--gaussian09",
		action="store_true",
		help="Uses 'module load g09' in the submission script, instead of 'module load g16' (the default).",
	)
	...
	for out_file in out_files:
		with open(out_file, mode="r") as out_reader:
			...
			make_input_file(job_name, input, random_seed, args.max_steps, args.memory, args.processors)
			if not args.no_script:
				make_submission_script(job_name, args.time, args.memory, args.processors, args.gaussian09)


def make_input_file(job_name, old_input, random_seed, max_steps, memory, procs):
	...
	for line in old_input:
		if "$job" in line.casefold():
			...
			random_seed_set = True
		elif in_job_section and max_steps is not None and "max_steps" in line.casefold():
			...
			if "bring_together" in line.casefold():
				...
				print(
					f"{job_name[:-4]}.out -- Warning: This script only "
