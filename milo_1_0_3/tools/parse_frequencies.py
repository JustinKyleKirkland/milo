...
def parse_gaussian_header(input_iterable, program_state):
	...
	for line in input_iterable:
		...
		if past_warning and "-----" in line:
			...
			tokens = [x for x in tokens if "#" not in x and "opt" not in x.casefold() and "freq" not in x.casefold()]
	...
	for line in input_iterable:
		...
		if "Input orientation:" in line or "Standard orientation:" in line:
			...
			for coordinate_line in input_iterable:
				if "Rotational constants" in coordinate_line or "Distance matrix" in coordinate_line:
					break
	...
	for line in input_iterable:
		if "Frequencies ---" in line:
			...
			for frequency in line.split()[2:]:
				program_state.frequencies.append(float(frequency), enums.FrequencyUnits.RECIP_CM)
		elif "Reduced masses ---" in line:
			for reduced_mass in line.split()[3:]:
				program_state.reduced_masses.append(float(reduced_mass), enums.MassUnits.AMU)
		elif "Force constants ---" in line:
			for force_constant in line.split()[3:]:
				program_state.force_constants.append(
					float(force_constant), enums.ForceConstantUnits.MILLIDYNE_PER_ANGSTROM
				)
		elif "Coord Atom Element:" in line:
			data_in_columns = list()
			for coordinate_line in input_iterable:
				if "Harmonic frequencies (cm**-1)" in coordinate_line or "                    " in coordinate_line:
					break
...
def print_job_section(output_iterable, program_state, verbose):
	...
	"""
	section = list()
	section.append(f"    gaussian_header         {program_state.gaussian_header}")
	if verbose >= 1:
		...
		section.append("    # phase                   bring_together n m  #  or  push_apart n m")
...
def print_molecule_section(output_iterable, program_state):
	...
	for _atom, (x, y, z) in zip(program_state.atoms, program_state.input_structure.as_angstrom()):
...
def print_frequency_data_section(output_iterable, program_state):
	...
	for frequency, reduced_mass, force_constant, mode_displacement in zip(
		...
		section.append(f"   {frequency:10.4f} {reduced_mass:7.4f} {force_constant:7.4f}")
