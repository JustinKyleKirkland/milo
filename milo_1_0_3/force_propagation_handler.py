...
def get_propagation_handler(program_state: ProgramState) -> Type["ForcePropagationHandler"]:
	...
	if program_state.propagation_algorithm is enums.PropagationAlgorithm.VERLET:
		return Verlet
	elif program_state.propagation_algorithm is enums.PropagationAlgorithm.VELOCITY_VERLET:
...
class ForcePropagationHandler:
	...
	@staticmethod
	def _calculate_acceleration(program_state: ProgramState) -> None:
	...
	@staticmethod
	def _calculate_velocity(program_state: ProgramState) -> None:
		...
		velocity = program_state.velocities[-1] + (
			containers.Velocities.from_acceleration(
				(program_state.accelerations[-1] + program_state.accelerations[-2]), program_state.step_size
			)
			* 0.5
		)
